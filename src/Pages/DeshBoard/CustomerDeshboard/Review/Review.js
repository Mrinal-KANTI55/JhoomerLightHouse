...
const Review = () => {
    ...
    useEffect(() => {
        axios.get(`https://fathomless-headland-44349.herokuapp.com/customerOrder/${user.email}`)
            .then(res => {
                setCustomerProducts(res.data);
            });
    }, [user.email]);
    const reviewInsert=e => {
        axios.post(`https://fathomless-headland-44349.herokuapp.com/reviewOrder`,UserReview)
            .then(res => {
            });
            e.preventDefault();
        }
