...
const ManageUserOrder = () => {
    ...
    useEffect(() => {
        axios.get('https://fathomless-headland-44349.herokuapp.com/customerOrder')
            .then(res => {
    ...
    const handelDelete = id => {
        ...
        if (conform) {
            const url = `https://fathomless-headland-44349.herokuapp.com/customerOrder/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
    ...
    const handelUpdateState = (id) => {
        // https://fathomless-headland-44349.herokuapp.com/ 
        fetch(`http://localhost:4000/customerOrder/${id}`, {
            method: 'PUT',
            headers: {
